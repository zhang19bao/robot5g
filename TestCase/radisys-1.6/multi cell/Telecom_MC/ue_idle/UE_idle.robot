*** Settings ***
Documentation     check ue idle mode under multicell
...
${cu_config_path}    /root/radisys-new/radisys-cu-1.6-E500-multicell/config
...
check UE attach
    ...
    Should Contain    ${output}=    Processing the DCCH msgType
    Write Bare    z
    ${output}=    Read    delay=100s
...
cu setup
    ...
    Write    sed -i 's/UE_INACTIVITY_TIMER = ${default_UE_INACTIVITY_TIMER}/UE_INACTIVITY_TIMER = ${new_UE_INACTIVITY_TIMER}/g' ./cell_config_2.txt
...
cu close
    ...
    Write    sed -i 's/UE_INACTIVITY_TIMER = ${new_UE_INACTIVITY_TIMER}/UE_INACTIVITY_TIMER = ${default_UE_INACTIVITY_TIMER}/g' ./cell_config_2.txt
    ${output}=    Read    delay=10s
