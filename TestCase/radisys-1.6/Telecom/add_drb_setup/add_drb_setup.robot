*** Settings ***
Documentation     check ue add additional drb - epc not support
...
UE attach
    ...
    Should Contain    ${output}=    Send RRC Reconfiguration Complete for UEId

do session request
    Write Bare    j
    ${output}=    Read    delay=60s
    Should Contain    ${output}=    UE Session Req triggered
    Should Contain    ${output}=    Send RRC Reconfiguration Complete for UEId
