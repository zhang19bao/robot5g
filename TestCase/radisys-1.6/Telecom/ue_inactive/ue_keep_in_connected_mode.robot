*** Settings ***
Documentation     check ue keep in connected mode
...
check whether UE change to inactivity
    check ue status
...
check ue status
    ...
    Should Not Contain    ${output}=    RRC Release Msg Rcvd
    Should Not Contain    ${output}=    isUeInactivity

keep ping traffic
    Open Connection    ${HOST}
    Login    ${USERNAME}    ${PASSWORD}
    Write    uecssh
    Write    nohup ping 12.12.12.13
    ${output}=    Read    delay=10s
