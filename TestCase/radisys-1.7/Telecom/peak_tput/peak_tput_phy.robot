*** Settings ***
Documentation     to check ue peak tput @ 1921.168.10.58 testline
Suite Setup       Testline Setup    # testline launch
Suite Teardown    Testline Close    # testline close
Library           SSHLibrary

*** Variables ***
${gNB HOST}       192.168.10.58
${core HOST}      192.168.10.59
${USERNAME}       root
${PASSWORD}       123456
${target_dl_tput}    50M
${target_ul_tput}    50M
${default_tti}    500000    #ms
${new_tti}        500000    #ms
...
check DL UDP traffic
    ...
    Login    ${USERNAME}    ${PASSWORD}
    Write    iperf3 -u -c 10.10.5.1 -B 12.12.12.113 -p 5001 -b ${target_dl_tput} -l 1024 -t 20
    ${output}=    Read    delay=30s
...
check UL UDP traffic
    Open Connection    ${core HOST}
    Login    ${USERNAME}    ${PASSWORD}
    Write    iperf3 -s -B 12.12.12.113 -p 5001
    Open Connection    ${gNB HOST}
    Login    ${USERNAME}    ${PASSWORD}
    Write    uecssh
    Write    iperf3 -u -c 12.12.12.113 -B 10.10.5.1 -p 5001 -b ${target_ul_tput} -l 1024 -t 20
    ${output}=    Read    delay=30s
    Should Contain    ${output}=    iperf Done
...
check ping traffic
    ...
    Login    ${USERNAME}    ${PASSWORD}
    Write    uecssh
    Write    ping 12.12.12.113 -I 10.10.5.1 -c 10
    ${output}=    Read    delay=12s
    Should Contain    ${output}=    64 bytes from 12.12.12.113
